...
package fr.jhelp.database.query.condition.booleans
...
internal class BooleanEquals<DO : DatabaseObject>(property: KProperty1<DO, Boolean>,
                                                  value: Boolean)
    : PropertyCondition<DO, Boolean>(property)
{
    override val type: DataType get() = DataType.BOOLEAN
    private val valueInt = if (value) 1 else 0

    override fun whereInternal(stringBuilder: StringBuilder)
    {
        stringBuilder.append(DatabaseNoSQL.COLUMN_VALUE_INTEGER)
        stringBuilder.append("=")
        stringBuilder.append(this.valueInt)
    }
}
